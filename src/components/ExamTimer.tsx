            ...
            <div className="absolute top-full right-0 mt-1 p-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg z-50 flex flex-col gap-2">
              ...
              {mode === 'countdown' && (
                <div className="flex items-center gap-1 text-xs">
                  <input type="number" min="1" max="180" value={countdownMin} onChange={e => setCountdownMin(parseInt(e.target.value) || 30)} className="w-12 px-1 py-0.5 border border-slate-200 dark:border-slate-600 dark:bg-slate-700 rounded text-center dark:text-white" /> 分钟
                </div>
              )}
            </div>
          )}
