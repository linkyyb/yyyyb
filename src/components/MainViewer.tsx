...
export default function MainViewer({ passage, onSentenceClick, onQuestionClick, onWordClick, phraseMode, phraseHighlights, onScanParagraph, onBookmark }: MainViewerProps) {
  ...
  const Header = () => (
    <div className="mb-8">
      ...
      <h2 className="text-2xl font-bold text-[var(--th-text)] mb-2">{passage.title}</h2>
      {passage.instruction && (
        <p className="text-sm text-[var(--th-text-soft)] italic bg-slate-50 border border-slate-100 p-4 rounded-lg">{passage.instruction}</p>
      )}
    </div>
  );
  ...
  const OptionBtn = ({ q, opt }: { q: any; opt: any }) => {
    ...
    let cls = 'border-slate-200 bg-[var(--th-bg-card)] hover:bg-blue-50 active:bg-blue-100 text-[var(--th-text)]';
    if (isRevealed) {
      ...
      else cls = 'border-slate-100 bg-[var(--th-bg-card)] text-slate-400';
    } else if (isSelected) {
    ...
    return (
      ...
      >
        <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${
          ...
          : 'bg-slate-200 text-[var(--th-text-soft)]'
        }`}>{opt.key}</span>
    ...
    return (
      <div className={`ml-10 p-4 rounded-lg mb-3 flex items-start gap-2 ${
        ...
        <div>
          ...
          {q.answerExplanation && (
            <p className="text-xs text-[var(--th-text-soft)] leading-relaxed">{q.answerExplanation}</p>
          )}
        </div>
      </div>
    );
  ...
  const QCard = ({ q, children = null }: { q: any; children?: React.ReactNode }) => {
    ...
    return (
      <div className={`bg-[var(--th-bg-card)] border rounded-xl p-6 shadow-sm relative group transition-all ${
        ...
        <div className="flex items-start gap-3 mb-4 pr-8">
          ...
          <div>
            <p className="font-bold text-[var(--th-text)] text-[15px]">{q.content}</p>
      ...
      <div className="p-8 pb-32">
            ...
            <div className="flex flex-wrap gap-2">
              {passage.wordBank.map((w, i) => (
                <span
                  ...
                  className="px-3 py-1.5 bg-[var(--th-bg-card)] border border-amber-300 rounded-full text-sm font-medium text-[var(--th-text)] hover:bg-amber-100 cursor-pointer transition-colors shadow-sm"
                >
        ...
        <div className="mb-10">
          <div className="flex items-center gap-2 mb-4">
            <AlignLeft className="w-4 h-4 text-[var(--th-text-soft)]" />
        ...
        {passage.questions.length > 0 && (
          <div className="border-t border-slate-200 pt-8">
            <h3 className="text-lg font-bold text-[var(--th-text)] mb-6 flex items-center gap-2">
    ...
    return (
      <div className="p-8 pb-32">
        ...
        <div className="mb-10 p-6 bg-slate-50 border border-slate-200 rounded-2xl">
          <div className="flex items-center gap-2 mb-4">
            <FileText className="w-4 h-4 text-[var(--th-text-soft)]" />
                  ...
                  <div className="space-y-1.5 mb-4 ml-10">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {Array.from({ length: 15 }, (_, i) => {
                        ...
                        let btnCls = 'border-slate-200 bg-[var(--th-bg-card)] text-[var(--th-text-soft)] hover:bg-slate-50';
                        if (isRev) {
                          ...
                          else btnCls = 'border-slate-100 bg-[var(--th-bg-card)] text-slate-300';
                        } else if (isSelected) {
      ...
      <div className="p-8 pb-32">
        ...
        {passage.questions.length > 0 && (
          <div className="mt-16 border-t border-slate-200 pt-8">
            <h3 className="text-xl font-bold text-[var(--th-text)] mb-6 flex items-center gap-2">
      ...
      <div className="p-8 pb-32">
        ...
        {passage.sourceText && (
          <div className="mb-8 p-6 bg-red-50/50 border border-red-200 rounded-2xl">
            ...
            <p className="text-lg leading-relaxed text-[var(--th-text)] font-serif whitespace-pre-wrap">{passage.sourceText}</p>
          </div>
        )}
        {passage.wordBank && passage.wordBank.length > 0 && (
          <div className="mb-8 p-5 bg-blue-50 border border-blue-200 rounded-2xl">
            ...
            <div className="flex flex-wrap gap-2">
              {passage.wordBank.map((w, i) => <span key={i} className="px-3 py-1 bg-[var(--th-bg-card)] border border-blue-200 rounded-lg text-sm text-[var(--th-text-soft)]">{w}</span>)}
            </div>
          </div>
        )}
        {passage.questions.length > 0 ? (
  ...
  return (
    <div className="p-8 pb-32">
      ...
      {passage.questions.length > 0 && (
        <div className="mt-16 border-t border-slate-200 pt-8">
          <h3 className="text-xl font-bold text-[var(--th-text)] mb-6">做题与精讲</h3>
