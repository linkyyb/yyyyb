...
export default function SettingsModal({ isOpen, onClose, apiKey, setApiKey, model, setModel, isThinkingMode, setIsThinkingMode, chatWordClick, setChatWordClick, wordGrammarAnalysis, setWordGrammarAnalysis }: SettingsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm">
      <div className="bg-[var(--th-bg-card)] w-[400px] rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <div className="p-4 border-b border-[var(--th-border)] flex justify-between items-center bg-[var(--th-bg-soft)]">
          ...
          <button onClick={onClose} className="p-1 text-slate-400 hover:text-[var(--th-text-soft)] rounded-md hover:bg-slate-200 transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 space-y-6">
          <div className="space-y-2">
            <label className="text-sm font-bold text-[var(--th-text)] flex items-center gap-2">
            ...
            <input 
              ...
              placeholder="sk-..."
              className="w-full px-4 py-2 bg-[var(--th-bg-soft)] border border-[var(--th-border)] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
            />
            <p className="text-[10px] text-slate-400">密钥仅存在您的本地浏览器中通过服务器转发请求，不作其他记录。</p>
          </div>

          <div className="space-y-2">
            <label className="text-sm font-bold text-[var(--th-text)]">请选择模型</label>
            <select 
              value={model}
              onChange={(e) => setModel(e.target.value as DeepSeekModel)}
              className="w-full px-4 py-2 bg-[var(--th-bg-soft)] border border-[var(--th-border)] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            >
          ...
          <div className="space-y-2 flex items-center gap-2">
            ...
            />
            <label htmlFor="thinkingCheckbox" className="text-sm font-bold text-[var(--th-text)] cursor-pointer">开启思考模式</label>
          </div>
          ...
          {setChatWordClick && (
            <div className="space-y-2 flex items-center gap-2">
              <input type="checkbox" id="chatWordCb" checked={chatWordClick}
                ...
                className="w-4 h-4 text-blue-600 rounded focus:ring-blue-500" />
              <label htmlFor="chatWordCb" className="text-sm font-bold text-[var(--th-text)] cursor-pointer">聊天框双击查词</label>
            </div>
          )}
            ...
            <div className="space-y-1.5">
              <div className="flex items-center gap-2">
                <input type="checkbox" id="wordGrammarCb" checked={wordGrammarAnalysis}
                  ...
                  className="w-4 h-4 text-blue-600 rounded focus:ring-blue-500" />
                <label htmlFor="wordGrammarCb" className="text-sm font-bold text-[var(--th-text)] cursor-pointer">单词弹窗显示词形语法解析</label>
              </div>
