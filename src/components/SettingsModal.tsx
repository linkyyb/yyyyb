...
export default function SettingsModal({ isOpen, onClose, apiKey, setApiKey, model, setModel, isThinkingMode, setIsThinkingMode, chatWordClick, setChatWordClick, wordGrammarAnalysis, setWordGrammarAnalysis }: SettingsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm">
      <div className="bg-white dark:bg-slate-800 w-[400px] rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <div className="p-4 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50">
          ...
          <button onClick={onClose} className="p-1 text-slate-400 hover:text-slate-600 dark:text-slate-300 rounded-md hover:bg-slate-200 transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="space-y-2">
            <label className="text-sm font-bold text-slate-700 dark:text-slate-200">请选择模型</label>
            <select 
              value={model}
          ...
          <div className="space-y-2 flex items-center gap-2">
            ...
            />
            <label htmlFor="thinkingCheckbox" className="text-sm font-bold text-slate-700 dark:text-slate-200 cursor-pointer">开启思考模式</label>
          </div>
          ...
          {setChatWordClick && (
            <div className="space-y-2 flex items-center gap-2">
              <input type="checkbox" id="chatWordCb" checked={chatWordClick}
                ...
                className="w-4 h-4 text-blue-600 rounded focus:ring-blue-500" />
              <label htmlFor="chatWordCb" className="text-sm font-bold text-slate-700 dark:text-slate-200 cursor-pointer">聊天框双击查词</label>
            </div>
          )}
            ...
            <div className="space-y-1.5">
              <div className="flex items-center gap-2">
                <input type="checkbox" id="wordGrammarCb" checked={wordGrammarAnalysis}
                  ...
                  className="w-4 h-4 text-blue-600 rounded focus:ring-blue-500" />
                <label htmlFor="wordGrammarCb" className="text-sm font-bold text-slate-700 dark:text-slate-200 cursor-pointer">单词弹窗显示词形语法解析</label>
              </div>
