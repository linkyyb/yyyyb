...
interface UploadVocabModalProps {
  isOpen: boolean;
  onClose: () => void;
  onUploadSuccess: (list: VocabList) => void;
  apiKey: string;
}

export default function UploadVocabModal({ isOpen, onClose, onUploadSuccess, apiKey }: UploadVocabModalProps) {
  const [file, setFile] = useState<File | null>(null);
  const [isUploading, setIsUploading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [uploadStage, setUploadStage] = useState<'idle' | 'extracting' | 'verifying' | 'parsing'>('idle');
  const [listTitle, setListTitle] = useState('My Vocabulary List');
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // 2. AI verification: fix OCR errors, spacing, garbled text
      ...
      } catch { /* Continue with original text if verify fails */ }

      const cleanTitle = listTitle.trim() || file.name.replace(/\.[^/.]+$/, "");

      setUploadStage('parsing');

      // 3. Queue chunk processing using TaskStore (runs in background)
          ...
          (result) => {
             if (result && result.words && result.words.length > 0) {
                const newList: VocabList = {
                  ...
                  title: cleanTitle,
                  createdAt: Date.now(),
                  words: result.words as WordItem[],
                };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm">
      <div className="bg-white w-[480px] rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
          ...
          <button onClick={handleClose} disabled={isUploading} className="p-1 text-slate-400 hover:text-slate-600 rounded-md transition-colors disabled:opacity-50">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 space-y-6">
          <div className="space-y-2">
            <label className="text-sm font-bold text-slate-700">词汇库名称</label>
            <input
              type="text"
              value={listTitle}
              ...
              className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-green-500 transition-all font-semibold text-slate-800 disabled:opacity-50"
            />
          </div>
          ...
          <div
             className={`border-2 border-dashed rounded-xl p-8 text-center transition-colors ${
               ...
               : 'border-slate-300 hover:border-green-400 hover:bg-slate-50 cursor-pointer'
             }`}
          ...
          >
            {isUploading ? (
               ...
               <div className="flex flex-col items-center gap-3">
                 ...
                 <p className="text-xs text-green-600 mb-2">已准备好提取 ({(file.size / 1024).toFixed(1)} KB)</p>
                 <button
                ...
                <>
                  ...
                  <p className="font-bold text-slate-700 mb-1">点击选择文件上传</p>
                  <p className="text-xs text-slate-500">支持 PDF、DOCX 或 TXT 格式</p>
                </>
            )}
            <input
              type="file"
          ...
          <button
            ...
            className="w-full py-3 bg-green-600 hover:bg-green-700 disabled:bg-slate-300 text-white font-bold rounded-lg text-sm transition-colors shadow-sm flex items-center justify-center gap-2"
          >
            {isUploading ? (
              <>
