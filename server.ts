...
function buildPrompt(s: SectionExtract): string {
  switch(s.type){
    case 'banked-cloze': return `Extract this CET-6 banked-cloze section. Extract ALL 15 word bank words (A-O labels). For each of the 10 blanks (26-35), create one question. CRITICAL: each blank's question should show ALL 15 word bank entries as options (A through O), NOT just 4. The user picks from the complete word bank. Deduce the correct letter for each blank from the passage context. Output ONLY JSON, no markdown. Format: {"title":"选词填空","section":"banked-cloze","wordBank":["word1","word2",...15 total],"paragraphs":[{"id":"p1","sentences":["text"]}],"questions":[{"id":"q26","number":26,"questionType":"banked-cloze","content":"为第26空选择最合适的单词","options":[{"key":"A","text":"word1"},{"key":"B","text":"word2"},...all 15],"correctAnswer":"H","answerExplanation":"中文解析"}]} You MUST output exactly 10 questions (26-35). Each MUST have exactly 15 options (A-O) matching the word bank.`;
